...
class ThreeDimModel():
    ...
    def load(self, dir):
        ...
        if self.builder:
            self.model = builders.builders_dict.get(
                self.builder)((256, 256, 5), self.num_labels)
        if exists(join(dir, 'saves')):
            if exists(join(dir, 'saves', 'weights.h5')):
                self.model.load_weights(
                    join(dir, 'saves', 'weights.h5'), by_name=True)
    ...
    def build_model(self):
        ...
        """
        if not self.model:
            self.model = self.model = builders.builders_dict.get(
                self.builder)((256, 256, 5), self.num_labels)
    ...
    def train(self, train_dir, val_dir, out_dir, batch_size, callbacks, metrics, learning_rate, epochs, decay, freeze=[]):
        ...
        train_gen = BatchGenerator(
            self, train_dir, batch_size).generate_training_batch()
        val_gen = BatchGenerator(
            self, val_dir, batch_size).generate_training_batch()
    ...
    def eval(self, val_dir, out_file, metrics):
        ...
        self.model.compile(optimizer, 'categorical_crossentropy', metrics)
        val_gen = BatchGenerator(
            self, val_dir, batch_size=1).generate_training_batch()
        evaluations = self.model.evaluate_generator(
            val_gen, dirsize(join(val_dir, 'RGB')))
        out_dict = {}
    ...
    def compute(self, data_dir, target_dir):
        ...
        data_gen = BatchGenerator(
            self, data_dir, batch_size=1).generate_inference_batch()
