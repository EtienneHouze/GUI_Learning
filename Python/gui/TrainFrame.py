...
class TrainFrame(tk.LabelFrame):
   ...
   Definition of the frame managing the creation, training and saving of a neural network model.
    ...
    def launch_training(self):
        ...
        self.model3D.train(self.train_folder.get(), self.val_folder.get(), self.model_folder.get(), batch_size=int(self.batch_size.get()), callbacks=[
        ], metrics=metrics, learning_rate=float(self.learning_rate.get()), epochs=int(self.epoch_number.get()), decay=float(self.decay_rate.get()), freeze=freeze)
