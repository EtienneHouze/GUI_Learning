...
class TrainFrame(tk.LabelFrame):
    ...
    def model_name_setter(self):
        ...
        print("Model has been named : "+self.model3D.name)
    ...
    def model_labels_setter(self):
        ...
        print("Model has been set to output "+str(self.model3D.num_labels)+ " labels.")

    def launch_training(self):
        if not self.model3D.builder:
            print("No builder was provided !")
            return
        if not self.model3D.name:
            print("Please enter a name for the model !")
            return
        if not self.model3D.num_labels:
            print("Please provide a valid number of labels")
            return
        ...
        if self.use_ckpt.get():
            callbacks.append(keras.callbacks.ModelCheckpoint(
                                                             join(self.model_folder.get(), 'ckpts.h5'),
        ...
        self.model3D.train(self.train_folder.get(),
                           self.val_folder.get(),
                           self.model_folder.get(),
                           batch_size=int(self.batch_size.get()), 
                           callbacks=callbacks, metrics=metrics,
                           learning_rate=float(self.learning_rate.get()), 
                           epochs=int(self.epoch_number.get()),
                           decay=float(self.decay_rate.get()), 
                           freeze=freeze
                           )
